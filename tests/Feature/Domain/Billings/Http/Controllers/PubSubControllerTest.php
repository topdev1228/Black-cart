...
use Tests\Fixtures\Values\Events\StoreCreatedEvent;
use Tests\TestCase;
...
class PubSubControllerTest extends TestCase
{
    public function testItHandlesEventPubSubMessages(): void
    {
        $this->app->singleton('App\Domain\Tests\Listeners\MockListener', fn () => new class($this) {
            ...
            public function handle(StoreCreatedEvent $event): void
            {
                $this->test->assertInstanceOf(BillingsStoreValue::class, $event->store);
                $this->test->assertNotEmpty($event->store->name);
                $this->test->assertNotEmpty($event->store->domain);
        ...
        $response = $this->postJson('/pubsub/stores/billings', $message);
        $response->assertOk();
    }
