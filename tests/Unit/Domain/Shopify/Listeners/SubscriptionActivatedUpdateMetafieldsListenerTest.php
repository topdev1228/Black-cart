...
use App\Domain\Shopify\Listeners\SubscriptionActivatedUpdateMetafieldsListener;
...
class SubscriptionActivatedUpdateMetafieldsListenerTest extends TestCase
{
    ...
    public function testItHandlesEvent(): void
    {
        Http::fake([
            App::context()->store->domain . '/admin/api/*' => Http::sequence()
                ...
                ->push(static::getShopifyMetafieldsSetForSubscriptionStatusChangedSuccessResponse('active')),
        ]);
        ...
        $event = SubscriptionActivatedEvent::from([
            'subscription' => SubscriptionValue::from($subscription),
        ]);

        $listener = resolve(SubscriptionActivatedUpdateMetafieldsListener::class);
